...
JDBCConn.prototype.executeQuery = function(sql,cb) {
  var self = this;

  self._conn.createStatement(function(err, statement) {
    if (err) {
      self.emit('executeQuery', err, null, null, cb);
    } else {
        ...
        if (err) {
          ...
          resultset.getMetaData(function(err,rsmd) {
            if (err) {
              ...
              self.emit('executeQuery', null, null, results, cb); 
            }
          });
        } else {
          self.emit('executeQuery', null, null, null, cb);
        }
      });
    }
  });
